...
func (m *DBModel) Get(id int) (*Movie, error) {
	...
	movie.MovieGenre, err = getGenres(ctx, m, id)
	if err != nil {
		return nil, err
	}
...
func (m *DBModel) All(genre ...int) ([]*Movie, error) {
	...
	query := fmt.Sprintf(` select id, title, description, year, release_date, runtime, mpaa_rating, created_at, updated_at from movies %s order by title`, where)
	...
	for rows.Next() {
		...
		genres, err := getGenres(ctx, m, movie.ID)
		if err != nil {
			return nil, err
		}
...
func getGenres(ctx context.Context, m *DBModel, id int) (map[int]string, error) {
	// get genres, if any
	query := ` select
